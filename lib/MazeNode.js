...
var MazeNode = /** @class */ (function () {
    ...
    MazeNode.prototype.connectTo = function (node, exitPosition, autoConnect) {
        ...
        if (this.getMaxExits() >= 0 || node.getMaxExits() >= 0) {
            if (!((this.getOccupiedExitPoints().length + 1 >= this.getMaxExits() || this.getMaxExits() == -1) &&
                (node.getOccupiedExitPoints().length + 1 >= node.getMaxExits() || node.getMaxExits() == -1))) {
                throw ("Illegal attempt to connect two nodes, at least one of which is already at maximum capacity.  You may" +
                    " try setting the maxExits value to -1 (signal to ignore max exits) or otherwise use helper methods" +
                    " to ensure valid state before invoking connectTo.");
            }
        }
    ...
    MazeNode.prototype.isPointOpen = function (position) {
        if (this.maxExits >= 0 && this.maxExits <= this.neighbors.length) {
            return false;
        }
