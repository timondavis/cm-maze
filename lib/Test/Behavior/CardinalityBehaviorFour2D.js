...
describe('Compass4', function () {
    ...
    it('can generate new neighboring coordinate objects, given a current coordinate object and a desired direction', function () {
        ...
        chai_1.expect(cb4.getNextLocation(spot, Compass4_1.C4.NORTH).getPosition().toString()).to.be.equal([0, -1].toString());
        chai_1.expect(cb4.getNextLocation(spot, Compass4_1.C4.EAST).getPosition().toString()).to.be.equal([1, 0].toString());
        chai_1.expect(cb4.getNextLocation(spot, Compass4_1.C4.SOUTH).getPosition().toString()).to.be.equal([0, 1].toString());
        chai_1.expect(cb4.getNextLocation(spot, Compass4_1.C4.WEST).getPosition().toString()).to.be.equal([-1, 0].toString());
    });
});
